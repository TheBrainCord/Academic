...
export default async function CoordinatorLayout({ children }: { children: React.ReactNode }) {
  ...
  if (!profile || !['coordinator', 'teacher'].includes(profile.role)) {
    redirect('/auth/login')
  }
  ...
  const nav = [
    ...
    { href: '/coordinator/supervisors', label: 'Supervisors' },
  ]
    ...
    <div className="min-h-screen bg-gray-50 flex">
      <aside className="w-56 bg-christ-navy text-white flex flex-col shrink-0">
        <div className="p-5 border-b border-white/10">
          ...
          <p className="font-semibold text-sm">Coordinator Portal</p>
        </div>
